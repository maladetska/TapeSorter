...
class Tape : public ITape<TapeType> {
 public:
  ...
  //   friend class TapeSorter<TapeType>;
...
void Tape<TapeType>::Put(const TapeType &element) {
  ...
  if (chunks_info_.chunks_number_ == 1) {
    PutElementInNewChunk(tmp_to, chunks_info_.max_size_chunk_, current_pos,
                         element);
    ...
    PutElementInNewChunk(tmp_to, chunks_info_.last_size_chunk_, current_pos,
                         element);
